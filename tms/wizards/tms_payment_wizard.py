...
class TmsPaymentWizard(models.TransientModel):
    _name = 'tms.payment.wizard'
    ...
    def _compute_amount_total(self):
        for rec in self:
            ...
            for obj in active_ids:
                if self._context.get('active_model') in ['tms.advance', 'tms.expense.loan']:
                    amount_total += currency.compute(obj.amount, self.env.user.currency_id)
                elif self._context.get('active_model') == 'tms.expense':
                    amount_total += currency.compute(obj.amount_balance, self.env.user.currency_id)

            rec.amount_total = amount_total

    def make_payment(self):
        for rec in self:
            active_ids = self.env[self._context.get('active_model')].browse(self._context.get('active_ids'))
            ...
            if len(currency_id) > 1:
                ...
                raise ValidationError(
                    _('You cannot pay documents in different currency of the bank (%s)' % rec.journal_id.currency_id.name))
            ...
            for obj in active_ids:
                ...
                if obj.state not in ['confirmed', 'closed'] or obj.paid:
                    raise ValidationError(
                        _('The document %s must be confirmed and unpaid.') % obj.name)
                ...
                model_amount = {
                    'tms.advance': obj.amount
                        if hasattr(obj, 'amount') and active_model == 'tms.advance' else 0.0,
                    'tms.expense': obj.amount_balance
                        if hasattr(obj, 'amount_balance') and active_model == 'tms.expense' else 0.0,
                    'tms.expense.loan': obj.amount
                        if hasattr(obj, 'amount') and active_model == 'tms.expense.loan'else 0.0
                }
            ...
            operating_unit_id = self.env['operating.unit'].search([], limit=1)
    ...
    def create_counterpart(self, model_amount, currency, obj, amount_currency, amount_bank, counterpart_move_line):
        for key, value in model_amount.items():
            if key == self._context.get('active_model'):
                if key == 'tms.expense' and value < 0.0:
                    raise ValidationError(
                        _('You cannot pay the expense %s because the balance is negative') % obj.name)
                else:
    ...
    def create_moves_and_reconciles(self, move, active_ids):
        ...
        for move_line in move_id.line_ids.filtered(lambda l: l.account_id.internal_group == 'liability'):
            move_ids = []
            ...
            if len(line) > 1:
                raise ValidationError(_('The driver advance account is defined as payable. %s ' % line[0].name))
