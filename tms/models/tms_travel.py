...
class TmsTravel(models.Model):
    ...
    def _compute_date_end(self):
        for rec in self:
            if rec.date_start:
                ...
                rec.date_end = strp_date + timedelta(hours=rec.route_id.travel_time)
            else:
                rec.date_end = False
    ...
    def _compute_travel_duration_real(self):
        for rec in self:
            if rec.date_start_real and rec.date_end_real:
                ...
                rec.travel_duration_real = False
    ...
    def validate_vehicle_insurance(self):
        ...
        xdays = int(val) or 0
        date = datetime.now().date() + timedelta(days=xdays)

        for rec in self:
            units = [rec.unit_id, rec.trailer1_id, rec.dolly_id, rec.trailer2_id]

            for unit in units:
                if (unit and unit.insurance_expiration and unit.insurance_expiration <= date):
                    raise ValidationError(_(
