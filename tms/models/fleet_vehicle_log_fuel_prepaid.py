...
class FleetVehicleLogFuelPrepaid(models.Model):
    ...
    _description = "Fleet Vehicle Log Fuel Prepaid"
    ...
    state = fields.Selection([
        ('draft', 'Draft'),
        ('confirmed', 'Confirmed'),
        ('closet', 'Closet')], readonly=True, default='draft', required=True)
    ...
    log_fuel_ids = fields.One2many('fleet.vehicle.log.fuel', 'prepaid_id',
        string='Fuel Vauchers', readonly=True,
    )
    ...
    def create_invoice(self):
        for rec in self:
            ...
            invoice_account = fpos.map_account(rec.vendor_id.property_account_payable_id)

            if rec.product_id.property_account_income_id:
                account = fpos.map_account(rec.product_id.property_account_income_id)
            elif (rec.product_id.categ_id.property_account_income_categ_id):
                account = fpos.map_account(rec.product_id.categ_id.property_account_income_categ_id)
            else:
                raise ValidationError(
            ...
            message = _('<strong>Invoice of:</strong> %s </br>') % (rec.name)
