    ...
    def create_mrp_from_pos(self, products):
        if products:
            ...
            for prod in products:
                print(prod)
                if not self.env['product.product'].browse(int(prod['id'])).to_make_mrp:
                    break 
                if prod['qty'] != 1: # for the coffeshop use case each mrp.production(manufacturingorde) should be created for each individual product, meaning only prod[qty] == 1 allowed
                    break
                    ...
                    if bom_count:
                        ...
                        if bom:
                            ...
                            for bom_line in mrp_order.bom_id.bom_line_ids:
                                bom_line_qty = self.env['mrp.bom'].search([("product_tmpl_id", "=", prod['product_tmpl_id'])]).product_qty
                                if(len(list(prod['components'])) > 0):
                                bom_line_qty =  * list(filter(lambda n: n['id'] == bom_line.product_id.id,  prod['components']))[0]['qty']
