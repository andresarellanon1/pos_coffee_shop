...
class ProductSpawnerScreen extends PosComponent {
    ...
    _computeExtras(event) {
        ...
        let bom_lines_variant = bom_lines.filter(line => line.bom_product_template_attribute_value_ids.every(att_id => selected_attributes_values_ids.includes(att_id)))
        ...
        for (let line of bom_lines) {
            console.log(`${line.product_id[1]}(${line.product_id[0]}) [${line.bom_product_template_attribute_value_ids}]`)
        }
