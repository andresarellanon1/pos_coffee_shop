...
class ProductSpawnerScreen extends PosComponent {
    ...
    _computeExtras(event) {
        ...
        console.log(bom_lines.map(element => element.bom_product_template_attribute_value_ids))
        ...
        let variant_bom_lines = bom_lines.filter(line => line.bom_product_template_attribute_value_ids.some(att_id => selected_attributes_ids.includes(att_id)))
        console.warn('variant_bom_lines')
        console.log(variant_bom_lines.map(element => element.bom_product_template_attribute_value_ids))
        let bom_lines_product_ids = variant_bom_lines.map(line => line.product_id[0])
        console.warn('extra_products_ids')
        console.log(extra_products.map(element => element.id))
        console.warn('variant_bom_lines_product_ids')
        console.log(bom_lines_product_ids)
        this.state.extra_components = extra_products.filter(extra => bom_lines_product_ids.includes(extra.id))
